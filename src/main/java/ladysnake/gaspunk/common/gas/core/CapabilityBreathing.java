...
public class CapabilityBreathing {
    ...
    public static final EntityAttribute MAX_AIR_SUPPLY = new ClampedEntityAttribute(null, GasPunk.MOD_ID + ".maxAirSupply", 300, 0, 1024).setDescription("Max Air Supply").setShouldWatch(true);

    static {
        MethodHandle temp = null;
        try {
            Method method = ReflectionHelper.findMethod(LivingEntity.class, "decreaseAirSupply", "func_70682_h", int.class);
    ...
    public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof LivingEntity) {
            event.addCapability(new Identifier(GasPunk.MOD_ID, "breathing_cap"), new Provider((LivingEntity) event.getObject()));
        }
    }
    ...
    public static class Provider implements ICapabilitySerializable<CompoundTag> {
        ...
        Provider(LivingEntity object) {
            ...
            if (object instanceof ServerPlayerEntity)
                this.instance = new PlayerBreathingHandler((ServerPlayerEntity) object);
            else {
        ...
        public CompoundTag serializeNBT() {
            return (CompoundTag) CAPABILITY_BREATHING.getStorage().writeNBT(CAPABILITY_BREATHING, instance, null);
        }
    ...
    public static class Storage implements Capability.IStorage<IBreathingHandler> {
        ...
        @Nullable
        @Override
        public Tag writeNBT(Capability<IBreathingHandler> capability, IBreathingHandler instance, EnumFacing side) {
            CompoundTag nbt = new CompoundTag();
            nbt.putFloat("breath", instance.getAirSupply());
            return nbt;
        }

        @Override
        public void readNBT(Capability<IBreathingHandler> capability, IBreathingHandler instance, EnumFacing side, Tag nbt) {
            instance.setAirSupply(((CompoundTag) nbt).getFloat("breath"));
        }
    }
}
