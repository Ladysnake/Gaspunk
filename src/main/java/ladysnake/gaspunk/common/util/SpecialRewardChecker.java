...
public class SpecialRewardChecker {
    ...
    private static Map<ServerPlayNetworkHandler, GrenadeSkins> playerSelectedSkins = Collections.synchronizedMap(new WeakHashMap<>());
    private static ImmutableMap<UUID, ImmutableSet<GrenadeSkins>> specialPersons = ImmutableMap.of();
    private static final ImmutableSet<GrenadeSkins> DEFAULT_SKINS = ImmutableSet.of(GrenadeSkins.NONE);
     ...
     */
    public static ImmutableSet<GrenadeSkins> getRewards(UUID uuid) {
     ...
     */
    public static void setSelectedGrenadeSkin(ServerPlayNetworkHandler playerHandler, GrenadeSkins skin) {
        // check that the player does have the selected reward
        if (isSpecialPerson(playerHandler.player.getUuid()) && getRewards(playerHandler.player.getUuid()).contains(skin)) {
     ...
     */
    public static GrenadeSkins getSelectedSkin(PlayerEntity player) {
        if (player == null) return GrenadeSkins.NONE;

        GrenadeSkins playerChoice = (player instanceof ServerPlayerEntity)
                ? playerSelectedSkins.get(((ServerPlayerEntity) player).networkHandler)
                : GasPunkConfig.get().getSelectedSkin();
        // double check to make sure that this player does have the skin he selected
        return getRewards(player.getUuid()).contains(playerChoice) ? playerChoice : GrenadeSkins.NONE;
    }
    ...
    public static void retrieveSpecialRewards() {
        try {
            ...
            GasPunkClient.onSpecialRewardsRetrieved();
        } catch (IOException e) {
     ...
     */
    @Nullable
    private static Pair<UUID, ImmutableSet<GrenadeSkins>> deserializeEntry(Map.Entry<String, List<String>> entry) {
        ImmutableSet.Builder<GrenadeSkins> builder = ImmutableSet.builder();
